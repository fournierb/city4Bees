...
p.TOP <- Make_PDP_plot_all(dat=pdp.TOP, ylabel="", labels=c(20,25,30,35))
p.TED<- Make_PDP_plot_all(dat=pdp.TED, ylabel="", labels=c(0.92, 0.93, 0.94))
p.FDis <- Make_PDP_plot_all(dat=pdp.FDis, ylabel="", labels=c(1.2, 1.4, 1.6, 1.8))
p.rich <- Make_PDP_plot_all(dat=pdp.rich, ylabel="", labels=c(8,10,12,14,16,18))
p.sha<- Make_PDP_plot_all(dat=pdp.sha, ylabel="", labels=c(1.4,1.5,1.6,1.7,1.8))
p.betataxo<- Make_PDP_plot_all(dat=pdp.betataxo, ylabel="", labels=c(1.59, 1.61, 1.63, 1.65, 1.67))
p.beta.fun<- Make_PDP_plot_all(dat=pdp.betafun, ylabel="", labels=c(0.15, 0.2, 0.25, 0.3, 0.35))


require(egg)
library(ggpubr)
figure.all <-  ggarrange(p.rich,p.sha,p.TOP,p.TED,p.FDis,p.betataxo,p.beta.fun,
                    nrow = 7, ncol=1, 
 #                   labels = paste("(",letters[1:7], ")", sep=""),
                    common.legend = F, legend="right", heights = c(1,1,1,1,1,1,1), widths = c(1,1,1,1,1,1,1))
figure.all
