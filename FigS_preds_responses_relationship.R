...
urban.df=as.data.frame(urban)
urban.df=cbind(urban.df, coordinates(masked.climate))
urban.df$coordsmerged=paste(urban.df$x, urban.df$y, sep="")
urban.df=urban.df[urban.df$coordsmerged %in% climate.df$coordsmerged ,]

beehive.df=as.data.frame(beehive)
beehive.df=cbind(beehive.df, coordinates(masked.climate))
beehive.df$coordsmerged=paste(beehive.df$x, beehive.df$y, sep="")
beehive.df=beehive.df[beehive.df$coordsmerged %in% climate.df$coordsmerged ,]
### Merge
dat=cbind(div.metrics,climate.df,vegetation.df,beehive.df[,1:8], urban.df[,4])
dat$ID= seq(from=1, to=nrow(dat))
### Make a sample
sample.id= sample(dat$ID, size = 10000)
...
p.lu = ggplot(data = dat.random,aes(y=get(paste(names(div)[r])), x = get('urban.df[, 4]'))) +
